...
class GenericAudioStudio(AudioStudioBase):
    ...
    async def generate_tts(self, text: str, output_path: str, emotion: str = None) -> bool:
        ...
        try:
            ...
            if hasattr(config, "PRONUNCIATION_FIXES") and config.PRONUNCIATION_FIXES:
                for target, replacement in config.PRONUNCIATION_FIXES.items():
                    if target in text:
                        text = text.replace(target, replacement)

            if config.ENABLE_EMOTIONAL_TTS and emotion and emotion != "neutral" and emotion != "serious":
                emotion_map = {
                ...
                speech_rate = config.get_speech_rate(config.CURRENT_CATEGORY)
